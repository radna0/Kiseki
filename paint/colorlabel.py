...
class ColorLabel:
    ...
    def extract_black_line(self, label_img, save_path=None, erosion_flag=True):
        ...
        line[line_mask] = [
            ...
            0,
            255,
        ]  # Change the background of 0 alpha region to white. It is designed for some softwares such as PaintMan.
    ...
    def generate_paired_data(self, color_img_path, label_img_path):
        ...
        return {
            ...
            "seg": seg,
            "json_color": color_dict,
            "json_index": index_dict,
        }
    ...
    def process_folder(self, load_folder, save_folder):
        ...
        os.makedirs(
            os.path.join(save_folder, "gt"), exist_ok=True
        )  # Use default setting to avoid deleting previous files
        ...
        if len(color_img_list) > 0:
            for i, color_img_path in tqdm(enumerate(color_img_list)):
                ...
                io.imsave(
                    os.path.join(save_folder, "gt/", str(i).zfill(4) + ".png"),
                    paired_data["gt"],
                    check_contrast=False,
                )
                io.imsave(
                    os.path.join(save_folder, "line_black/", str(i).zfill(4) + ".png"),
                    paired_data["line_black"],
                    check_contrast=False,
                )
                io.imsave(
                    os.path.join(save_folder, "seg/", str(i).zfill(4) + ".png"),
                    paired_data["seg"],
                    check_contrast=False,
                )
                dump_json(
                    paired_data["json_color"],
                    os.path.join(save_folder, "json_color/", str(i).zfill(4) + ".json"),
                )
                dump_json(
                    paired_data["json_index"],
                    os.path.join(save_folder, "json_index/", str(i).zfill(4) + ".json"),
                )
        else:
...
""" def relabel_image(label_img, color_dict):
    ...
    sorted_labels = sorted(
        label_leftup_pix_dict, key=lambda k: label_leftup_pix_dict[k][0]
    )
...
def compute_top_left_coords(label_img, max_label):
    ...
    def extract_label_folder(
        self, load_folder, save_folder, color_folder_name="gt", extract_seg=False
    ):
        # If fill single pixel is true, it will
        os.makedirs(
            os.path.join(save_folder, "seg"), exist_ok=True
        )  # Use default setting to avoid deleting previous files
        color_img_list = sorted(
            glob(os.path.join(load_folder, color_folder_name, "*.png"))
        )
        ...
        if len(color_img_list) > 0:
            for i, color_img_path in tqdm(enumerate(color_img_list)):
                ...
                self.extract_label_map(
                    color_img_path, label_img_path, line_img_path, extract_seg
                )
        else:
            assert False, "Error! No images are loaded."
