...
class LineArt:
    def __init__(self, lineart_img, colorbook=None, new_colorbook=None):
        ...
        self.label_hightlight_shadow = [1] * (
            self.label_img.max() + 1
        )  # 0 means highlight, 1 means normal and 2/3 means shadow1/2
...
def trappedball_fill(img_path, save_path, radius=4, contour=False):
    ...
    radii = [max(10, radius), max(5, radius // 2), 1]
