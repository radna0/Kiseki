...
class CLIPEncoder(nn.Module):
    def __init__(self, freeze=True, resolution=None):
        ...
        self.visual_encoder = torch.nn.Sequential(
            visual_model0, *list(visual_model1)[:-2]
        )
...
class DeformableConv2d(nn.Module):
    ...
    def forward(self, x, use_offset=False):
        ...
        if c != 6:
            assert (
                False
            ), "input tensor should has 6 channels with 3 as line art and 3 as color."
        ...
        if use_offset:
            ...
            color_fea = torchvision.ops.deform_conv2d(
                input=color_fea,
                offset=offset,
                weight=self.color_conv.weight,
                bias=self.color_conv.bias,
                padding=self.padding,
                mask=mask,
            )
        else:
...
class UNet(nn.Module):
    ...
    def forward(self, x, line=None, use_offset=False):
        ...
        if self.use_clip:
            ...
            x_clip_fea_resized = F.interpolate(
                x_clip_fea,
...
class SegmentDescriptor(nn.Module):
    ...
    def forward(self, img, seg, line=None, use_offset=False):
        ...
        if self.encoder_resolution:
            img = F.interpolate(
                img, self.encoder_resolution, mode="bilinear", align_corners=False
            )
            line = (
                F.interpolate(line, self.encoder_resolution, mode="nearest")
                if line is not None
                else None
            )
