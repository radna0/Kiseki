...
class PPM(nn.Module):
    def __init__(self, in_dim, reduction_dim, bins):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(
                in_dim + reduction_dim * 4, in_dim, kernel_size=3, padding=1, bias=False
            ),
            nn.PReLU(),
        )
...
class UNet(nn.Module):
    def __init__(
        self, enc_dim, ch_in=3, use_ppm=False, use_clip=False, bins=[1, 2, 3, 6]
    ):
    ...
    def forward(self, x):
        ...
        x3 = torch.cat(
            (x3, x2_resized1, x1_resized2), dim=1
        )  # 112 (64+32+16) ,H/4, H/4
        x4 = torch.cat(
            (x4, x3_resized1, x2_resized2, x1_resized3), dim=1
        )  # 240 (128+64+32+16) ,H/8, H/8
        x5 = torch.cat(
            (x5, x4_resized1, x3_resized2, x2_resized3, x1_resized4), dim=1
        )  # 496 (256+128+64+32+16) ,H/16, H/16
...
class TagFeatureEncoder(nn.Module):
    def __init__(self, enc_dim):
        ...
        text_linear_path = (
            "/data/dyk/Segment_Matching/BasicPBC_release/BasicPBC/ckpt/text_linear.pth"
        )
    ...
    def forward(self, x):
        ...
        one_hot = (
            torch.nn.functional.one_hot(
                torch.tensor(indices), num_classes=len(all_text_labels)
            )
            .float()
            .unsqueeze(0)
...
class BasicPBC_ref(nn.Module):
    ...
    def __init__(
        ...
        config.raft_resolution = (1024, 1024)
        ...
        # stage1
        self.desc_line = SegmentDescriptor(
            self.config.descriptor_dim, self.config.ch_in, use_ppm=True, use_clip=False
        )
        self.desc_parse = SegmentDescriptor(
            self.config.descriptor_dim,
            self.config.ch_in_ref,
            use_ppm=True,
            use_clip=False,
        )
        ...
        if self.config.wo_parsing and self.config.wo_text:
            ...
            self.fuse_tar = Fuse(
                2 * self.config.descriptor_dim, self.config.descriptor_dim
            )
    ...
    def forward(self, data):
        ...
        if not self.config.wo_parsing:
            ...
            seq_parse_tar = self.desc_parse(input_parse_tar, data["segment"])[
                ..., 1:
            ]  # 1, d, n
        else:
            seq_parse_tar = self.desc_parse(input_tar, data["segment"])[
                ..., 1:
            ]  # 1, d, n

        # ref
        input_ref = data["line_ref"]
        if not (self.config.wo_parsing and self.config.wo_text):
            seq_tag_ref = self.clip_text(data["used_tags_ref"])  # 1, d, m
            # print("seg_tag", seq_tag_ref, seq_tag_ref.shape)
            seg_tag_indices = [
                data["used_tags_ref"].index(tag) for tag in data["seg_tags_refs"]
            ]  # indices of the segment tags

        if self.config.use_raft:
            h, w = input_tar.shape[-2:]
            color_ref = data["recolorized_img"]
            line_tar = F.interpolate(
                input_tar,
            ...
            color_ref_resize = F.interpolate(
                color_ref,
                self.config.raft_resolution,
                mode="bilinear",
                align_corners=False,
            ...
            color_tar = flow_warp(
                color_ref_resize, flow_up.permute(0, 2, 3, 1).detach(), "nearest"
            )
            color_tar = F.interpolate(
                color_tar, (h, w), mode="bilinear", align_corners=False
            )
        ...
        if not (self.config.wo_parsing and self.config.wo_text):
            ...
            text_scores = transport(
                logits / self.config.descriptor_dim**0.5, self.bin_score
            )[:, :-1, :].squeeze(
                0
            )  # n, m+1
            ...
            if tag_all_matches is not None and self.config.text_loss_weight > 0:
                ...
                text_loss = nn.functional.cross_entropy(
                    text_scores, tag_all_matches.long().view(-1), reduction="mean"
                )
        ...
        if self.config.wo_parsing and self.config.wo_text:
            ...
            seq_segment_ref = self.desc_line(input_ref, data["segment_ref"])[
                ..., 1:
            ]  # 1, d, m (num of ref segments)
            ...
            # ref
            seq_segment_ref = self.desc_line(input_ref, data["segment_refs"])[
                ..., 1:
            ]  # 1, d, m (num of ref segments)

            if self.config.wo_text:
                seq_segment_fuse_ref = self.fuse_ref(
                    seq_segment_ref.permute(0, 2, 1)
                ).permute(
                    0, 2, 1
                )  # 1, d, m
            else:
                seq_segment_cat_ref = torch.cat(
                    [seq_segment_ref, seq_tag_ref[:, :, seg_tag_indices]], dim=1
                )
                seq_segment_fuse_ref = self.fuse_ref(
                    seq_segment_cat_ref.permute(0, 2, 1)
                ).permute(
                    0, 2, 1
                )  # 1, d, m
        ...
        if kpts.shape[1] < 2 or kpts_ref.shape[1] < 2:  # no keypoints
            ...
            return {
                "matches0": kpts.new_full(shape0, -1, dtype=torch.int)[0],
                "matching_scores0": kpts.new_zeros(shape0)[0],
                "skip_train": True,
            }
        ...
        all_matches_origin = (
            seg_all_matches.clone() if seg_all_matches is not None else None
        )
        ...
        if seg_all_matches is not None:
            return {
                ...
                "text_scores": (
                    text_scores if self.config.text_loss_weight > 0 else None
                ),
                "pred_labels": (
                    pred_text_labels if self.config.text_loss_weight > 0 else None
                ),
                ...
                "skip_train": False,
                "accuracy": (
                    (all_matches_origin[0] == indices0[0]).sum()
                    / len(all_matches_origin[0])
                ).item(),
                """ "area_accuracy": (
                    torch.tensor(
                        [
                            (data["segment"] == ii + 1).sum()
                            for ii in torch.arange(
                                0, all_matches_origin[0].shape[0]
                            ).to(data["segment"])[all_matches_origin[0] == indices0[0]]
                        ]
                    ).sum()
                    / (weights.sum() * 1.0)
                ).item(), """
                "valid_accuracy": (
                    (
                        (all_matches_origin[0] == indices0[0])
                        & (all_matches_origin[0] != -1)
                    ).sum()
                    / (all_matches_origin[0] != -1).sum()
                ).item(),
                "invalid_accuracy": (
                    (
                        (
