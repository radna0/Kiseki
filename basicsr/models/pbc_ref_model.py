...
from kiseki.paint import recolorize_img
...
class PBCModel_ref(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if "valid_acc" in pred:
            loss_dict["valid_acc"] = torch.tensor(pred["valid_accuracy"]).to(
                self.device
            )
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        gt_folder_path = dataloader.dataset.opt[
            "root"
        ]  # self.opt['datasets']['val']['root']
        ...
        json_color_folder_name = (
            "json_color"
            if dataset_name in ["PaintBucket_Char", "PaintBucket_Char_val"]
            else "seg"
        )
        ...
        results = eval_json_folder(
            save_path,
            gt_folder_path,
            stage=stage,
            json_folder_name=json_color_folder_name,
        )
        ...
        if save_csv:
            ...
            avg_dict, _, _ = evaluate(
                results, mode=dataset_name, skip_first=skip_first, stage=stage
            )
...
class PBCModelStage1(SRModel):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        json_folder_name = (
            "seg"
            if dataset_name
            in ["PaintBucket_Char_test", "PaintBucket_Char_debug", "PaintBucket_Real"]
            else "json_color"
        )
        ...
        results = eval_json_folder(
            save_path, gt_folder_path, stage="stage1", json_folder_name=json_folder_name
        )
...
class ModelInference:
    ...
    def process_stage2_frame_by_frame(self, batch, pred):
        ...
        prev_json_path = osp.join(
            osp.split(root)[0], load_folder, prev_name_str + ".json"
        )
        if is_val:
            save_folder = osp.join(save_path, osp.split(osp.split(root)[0])[-1])
        else:
            save_folder = osp.join(
                osp.split(root)[0].replace("test_data", "results"), save_path
            )
        save_folder = osp.join(save_folder, "stage2")
        start_frames = (
            [0]
            if "PaintBucket_Real" in root
            else [idx for idx in range(0, 10000, frame_per_clip)]
        )
        if prev_index in start_frames:
            mkdir(save_folder)
            shutil.copy(
                prev_json_path, save_folder
            )  # Copy the 0000.json to the result folder
            if save_img or self_prop:
                label0_path = osp.join(
                    osp.split(root)[0], "seg", prev_name_str + ".png"
                )
        ...
        if self_prop:
            ...
            prev_color_img = io.imread(osp.join(save_folder, prev_name_str + ".png"))[
                :, :, :3
            ]
            ...
            recolorized_img = (
                torch.from_numpy(prev_color_img).permute(2, 0, 1).float() / 255.0
            )
    ...
    def process_stage2_based_on_reference(self, batch, pred):
        ...
        ref_json_color_path = osp.join(
            osp.split(root)[0], json_color_folder_name, ref_name_str + ".json"
        )
        if is_val:
            save_folder = osp.join(save_path, osp.split(osp.split(root)[0])[-1])
        else:
        ...
        if index == "label":
            ref_json_index_path = ref_json_color_path.replace(
                json_color_folder_name, "json_index"
            )
            index_dict = load_json(ref_json_index_path)
            index_dict = {int(idx): pair[-1] for idx, pair in index_dict.items()}
            index_dict = {
                label_idx: seg_idx for seg_idx, label_idx in index_dict.items()
            }
        ...
        if not accu:
            ...
            for i, scores in enumerate(match_scores):
                ...
                if index == "seg":
                    color_lookup = np.array(
                        [
                            color_dict[idx] if idx in color_dict else [0, 0, 0, 0]
                    ...
                    color_lookup = np.array(
                        [
                            color_dict[index_dict.get(label_idx, 1)]
                            for label_idx in batch["seg_list_refs"][0].tolist() + [-1]
                        ]
                    )
                unique_colors = np.unique(color_lookup, axis=0)
                accumulated_probs = [
    ...
    def process_stage1(self, batch, pred):
        ...
        if save_img:
            ...
            colorize_label_image(
                label_save_path, json_save_path, img_save_path, using="label"
            )
    ...
    def visualize_text_matching(self, save_path):
        ...
        with torch.no_grad():
            ...
            for data in tqdm(self.test_loader):
                ...
                save_folder = osp.join(
                    save_path, osp.split(osp.split(root)[0])[-1], name_str
                )
                mkdir(save_folder)
                # match_tensor = self.model(self.dis_data_to_cuda(data))
                match_tensor = self.model(data)
                text_match_scores = match_tensor[
                    "text_scores"
                ]  # b, n(number of segments), m(number of tags)
