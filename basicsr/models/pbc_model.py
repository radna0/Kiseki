...
class ModelInference:
        ...
        with torch.no_grad():
            ...
            for test_data in tqdm(self.test_loader):
                ...
                if prev_index == 0:
                    ...
                    if save_img:
                        gt0_path = osp.join(
                            character_root, "ref", prev_name_str + ".png"
                        )
        ...
        with torch.no_grad():
            ...
            for test_data in tqdm(self.test_loader):
                ...
                if character_name not in characters:
                    ...
                    logger.info(glob(osp.join(gt_root, "*.png")))
                    for gt_path in glob(osp.join(gt_root, "*.png")):
                        ...
                        if keep_line:
                            line_path = gt_path.replace("ref", "line")
                            merged_img_save_path = gt_path.replace(
                                "ref", character_name + "_keepline"
                            )
                ...
                if keep_line:
                    ...
                    merged_img_save_path = osp.join(
                        save_folder_keepline, name_str + ".png"
                    )
                    merge_color_line(line_path, img_save_path, merged_img_save_path)
                    logger.info(f"image saved at:  {merged_img_save_path}")
    ...
    def inference_multi_gt_parallel(self, save_path, keep_line=False):
        # this function will be used to parallelize the inference process
        save_folder = osp.join(save_path, "seg")
        save_folder_keepline = osp.join(save_path, "seg_keepline")
        characters = set()
        os.makedirs(save_folder, exist_ok=True)
        if keep_line:
            os.makedirs(save_folder_keepline, exist_ok=True)
    ...
    def inference_multi_gt_sequential(self, save_path, keep_line=False):
        ...
        for idx, test_data in enumerate(self.test_loader):
            self.inference_single_frame(
                test_data,
                save_path,
                keep_line,
