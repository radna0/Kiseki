...
from kiseki.paint import load_json, read_img_2_np, read_seg_2_np, recolorize_seg
...
class LineSegAugmentor:
    ...
    def resize(self, image, resize_size):
        resized_image = cv2.resize(
            image, (resize_size, resize_size), interpolation=cv2.INTER_NEAREST
        )
    ...
    def __call__(self, line, seg):
        ...
        angle = (
            np.random.uniform(-self.rotate_degree, self.rotate_degree)
            if apply_rotation
            else 0
        )
...
class AnimeLabelSegDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.shuffle_label:
            ...
            label_ref_list = (
                np.concatenate(([-1], label_ref_list[label_ref_list != -1]))
                if -1 in label_ref_list
                else label_ref_list
            )
        ...
        mat_index = [
            (
                label_ref_list.index(seg_label_idx_map[x])
                if seg_label_idx_map[x] in label_ref_list
                else -1
            )
            for x in seg_list
        ]
        ...
        for i, label_idx in enumerate(label_ref_list):
            ...
            if merge:
                ...
                selected_label = np.random.randint(
                    2, label_ref_index
                )  # 0 means black line, and 1 means background
            else:
...
class KisekiLabelSegDataset(AnimeLabelSegDataset):
    def __init__(self, opt):
        # This class is mainly for training.
        aug_params = opt["aug_params"] if "aug_params" in opt else None
        super(KisekiLabelSegDataset, self).__init__(aug_params)

        self.opt = opt
        self.root = opt["root"]
        self.shuffle_label = opt["shuffle_label"] if "shuffle_label" in opt else False
        self.color_redistribution_type = (
            opt["color_redistribution_type"]
            if "color_redistribution_type" in opt
            else None
        )
        assert self.color_redistribution_type in [None, "seg"]
        self.merge_label_prob = (
            opt["merge_label_prob"] if "merge_label_prob" in opt else 0.0
        )
        self.frame_distance_list = (
            opt["frame_distance_list"] if "frame_distance_list" in opt else [1]
        )
