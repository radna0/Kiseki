...
class SegMatAugmentor:
    def __init__(
        self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=True, aug_half=False
    ):
    ...
    def spatial_transform(self, img1, img2, seg1, seg2):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            if self.aug_half and x < self.stretch_prob:
                ...
                seg1 = cv2.resize(
                    seg1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST
                )
            ...
            seg2 = cv2.resize(
                seg2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST
            )
    ...
    def __init__(self, opt):
        ...
        for character in os.listdir(root):
            ...
            assert len(line_list) == len(
                seg_list
            ), "line number should match the seg number"
            assert len(line_list) == len(
                gt_list
            ), "line number should match the gt number"
