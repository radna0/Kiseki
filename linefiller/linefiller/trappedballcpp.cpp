...
py::array_t<uint8_t> cv_mat_to_numpy_uint8(const cv::Mat &mat) {
  CV_Assert(mat.type() == CV_8UC1);
  return py::array_t<uint8_t>(
      {mat.rows, mat.cols},                         // shape
      {mat.step[0], mat.step[1]},                   // strides
      mat.data,                                     // data pointer
      py::capsule(mat.data, [](void *) {})          // no-op deleter
  );
}
...
PYBIND11_MODULE(trappedballcpp, m) {
  ...
  m.def("trapped_ball_fill_single", [](py::array_t<uint8_t> image_np,
                                       std::pair<int, int> seed,
                                       int radius) {
    cv::Mat image = numpy_uint8_to_cv_mat(image_np);
    cv::Point seed_point(seed.second, seed.first); // (x, y) = (col, row)
    cv::Mat filled = trappedBallFillSingle(image, seed_point, radius);
    return cv_mat_to_numpy_uint8(filled);
  }, py::arg("image"), py::arg("seed"), py::arg("radius"),
     R"pbdoc(
        Perform a single trapped-ball fill from a seed point.

        Parameters:
            image (np.ndarray): Input binary image (uint8, values 0 or 255).
            seed (Tuple[int, int]): Seed point as (row, col).
            radius (int): Radius of the ball structuring element.

        Returns:
            np.ndarray: Resulting filled image (uint8).
     )pbdoc");
  m.def(
      "flood_fill_multi",
