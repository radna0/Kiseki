...
class ColorizationPipeline:
    ...
    def process(self, img_path: str) -> np.ndarray:
        ...
        for r in radii:
            ...
            num_labels_new, labels_map_new, stats_new, _ = (
                cv2.connectedComponentsWithStats(
                    closed_binary, connectivity=8, ltype=cv2.CV_32S
                )
            )

            difference_mask = (previous_map == 0) & (labels_map_new > 0)

            if np.any(difference_mask):
                new_labels = labels_map_new[difference_mask]
                final_map[difference_mask] = new_labels

            previous_map = labels_map_new
        ...
        # --- FIX IS HERE: Removed the extra 'stats' argument ---
        merged_map = self.merge_regions_iteratively(final_map, region_graph)
