...
def build_graph_numba(labels_map: np.ndarray):
    """
    Numba-accelerated function to build the RAG by checking a 3x3 neighborhood.
    This is robust and avoids data type errors by working directly on the int32 map.
    """
    graph_counts = {}  # Numba requires explicit handling of dicts.
    h, w = labels_map.shape
    # Iterate from 1 to h-1 and 1 to w-1 to stay within bounds for the 3x3 window.
    for y in range(1, h - 1):
        for x in range(1, w - 1):
            ...
            # Check the 3x3 window around the center pixel
            for dy in range(-1, 2):
                for dx in range(-1, 2):
                    if dx == 0 and dy == 0:
                        continue
                    ...
                    if p_neighbor != 0 and p_center != p_neighbor:
                        # Create a canonical edge key (smaller_id, larger_id)
                        edge = (
                            (p_center, p_neighbor)
                        ...
                        # Explicitly handle dict updates in a Numba-friendly way
...
class ColorizationPipeline:
    ...
    def build_region_adjacency_graph(self, labels_map: np.ndarray):
        ...
        print("[INFO] Building topologically-correct Region Adjacency Graph...")
        # This function is now JIT-compiled and robust, with no invalid data types.
