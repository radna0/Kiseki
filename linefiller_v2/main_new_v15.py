...
class ColorizationPipeline:
    ...
    def watershed_segmentation(self, line_art_binary: np.ndarray):
        ...
        # Find ALL potential regions first to guarantee no region is lost.
        num_components, all_components = cv2.connectedComponents(line_art_binary)
        # Add 1 because watershed uses 0 as a boundary. This becomes our marker map.
        markers = all_components.astype(np.int32) + 1
        # Mark the line art area as the 'unknown' region for watershed to solve.
        markers[self.original_line_mask] = 0
        # Run the watershed.
        markers = cv2.watershed(self.original_bgr, markers)
        # Final boundaries become lines.
        markers[markers == -1] = 0
        return markers
