...
class ColorizationPipeline:
    def __init__(self, min_merge_area=100, erosion_iterations=2):
    ...
    # --- NEW: Building the pipeline's nervous system ---
    def build_region_adjacency_graph(self, labels_map: np.ndarray):
        ...
        # Scan horizontally and vertically to find neighbors
        # This is faster than dilating every single region
        ...
        for y in range(h - 1):
            for x in range(w - 1):
                ...
                p2 = map_padded[y, x + 1]  # Check right neighbor
                if p2 != 0 and p1 != p2:
                    # Sort to make the edge key canonical (e.g., (1,2) is same as (2,1))
                    edge = tuple(sorted((p1, p2)))
                    graph[edge] += 1

                p3 = map_padded[y + 1, x]  # Check bottom neighbor
                if p3 != 0 and p1 != p3:
                    edge = tuple(sorted((p1, p3)))
                    graph[edge] += 1
        return graph

    # --- NEW: The intelligent, iterative merging algorithm ---
    def merge_regions_iteratively(
        self, labels_map: np.ndarray, graph: dict
    ) -> np.ndarray:
        """Merges small regions into their best neighbors based on the RAG until no more merges can be made."""
        print(
        ...
        while True:
            num_labels, _, stats, _ = cv2.connectedComponentsWithStats(
                (merged_map > 0).astype(np.uint8)
            )

            # Find small regions in the *current* state of the map
            small_regions = {
                label: stats[label, cv2.CC_STAT_AREA]
                for label in range(1, num_labels)
                if stats[label, cv2.CC_STAT_AREA] < self.min_merge_area
            }

            if not small_regions:
                print("[INFO] Merging complete. No more small regions found.")
                break

            print(
                f"[INFO] Found {len(small_regions)} small regions to process in this pass."
            )

            merges_made_in_pass = 0
            # Sort small regions to process smallest first
            sorted_small_labels = sorted(
                small_regions.keys(), key=lambda l: small_regions[l]
            )

            # Create a dynamic mapping for merges
            merge_target = {}

            for label_id in sorted_small_labels:
                ...
                for edge, border_length in graph.items():
                    if label_id in edge:
                        neighbor = edge[0] if edge[1] == label_id else edge[1]
                ...
                if best_neighbor != -1:
                    ...
                    merge_target[label_id] = best_neighbor
                    merges_made_in_pass += 1

            if merges_made_in_pass == 0:
                print("[INFO] No valid merges found in this pass. Terminating.")
                break

            # Apply all merges for this pass
            # This requires re-labeling the map, which is slow. A better way is to update a mapping.
            # For simplicity here, we'll remap directly.
            for from_id, to_id in merge_target.items():
                merged_map[merged_map == from_id] = to_id
    ...
    def process(self, img_path: str) -> np.ndarray:
        ...
        # The new graph-based workflow
        region_graph = self.build_region_adjacency_graph(combined_map)
        merged_map = self.merge_regions_iteratively(combined_map, region_graph)
