...
def get_unfilled_point(image: np.ndarray):
    ...
    # Returns
        an array of points.
    """
    y, x = fast_where(image, 255)

    # return np.stack((x.astype(int), y.astype(int)), axis=-1)
    # # Stack the coordinates and convert to integers
...
def exclude_area(image: np.ndarray, radius: int):
    ...
    return cv2.morphologyEx(
        image,
        cv2.MORPH_ERODE,
        get_ball_structuring_element(radius),
        anchor=(-1, -1),
        iterations=1,
    )
...
def trapped_ball_fill_single(image: np.ndarray, seed_point: tuple, radius: int):
    ...
    pass1 = fast_full_255_uint8(image.shape)
    pass2 = fast_full_255_uint8(image.shape)
    ...
    pass1 = cv2.morphologyEx(
        pass1, cv2.MORPH_DILATE, ball, anchor=(-1, -1), iterations=1
    )
    ...
    pass2 = cv2.morphologyEx(
        pass2, cv2.MORPH_ERODE, ball, anchor=(-1, -1), iterations=1
    )
...
@njit(nogil=True)
def fast_where(fill: np.ndarray, target: int):
    return np.where(fill == target)


@njit(nogil=True)
def fast_full_255_uint8(shape):
...
def trapped_ball_fill_multi(
    ...
    # Arguments
        ...
        method: method for filtering the fills.
               ...
               'median' is usually with small radius for select small area such as lines.
               'mean' is usually with small radius for select small area such as lines.
        max_iter: max iteration number.
    # Returns
        an array of fills' points.
    """
    # logger.info(f"trapped-ball filling with radius: {radius}")
...
def flood_fill_single(im: np.ndarray, seed_point: tuple):
    ...
    # Returns
        an image after filling.
    """
    pass1 = fast_full_255_uint8(im.shape)
...
def flood_fill_multi(image: np.ndarray, max_iter: int = 20000):
    ...
    for _ in range(max_iter):
        ...
        filled_area.append(fast_where(fill, 0))
...
def get_bounding_rect(points: list):
    ...
    # Returns
        rect coord
    """
    return np.min(points[1]), np.min(points[0]), np.max(points[1]), np.max(points[0])
...
def get_border_point(points: tuple, rect: list, max_height: int, max_width: int):
    ...
    # Get fill in rect.
    fill = np.zeros(
        (border_rect[3] - border_rect[1], border_rect[2] - border_rect[0]), np.uint8
    )
    ...
    approx_shape = cv2.approxPolyDP(
        contours[0], 0.02 * cv2.arcLength(contours[0], True), True
    )
    ...
    border_pixel_mask = (
        cv2.morphologyEx(fill, cv2.MORPH_DILATE, cross, anchor=(-1, -1), iterations=1)
        - fill
    )
    border_pixel_points = fast_where(border_pixel_mask, 255)

    # Transform points back to fillmap.
    border_pixel_points = (
        border_pixel_points[0] + border_rect[1],
        border_pixel_points[1] + border_rect[0],
    )
...
def merge_fill(fillmap: np.ndarray, max_iter: int = 10):
    ...
    for i in range(max_iter):
        ...
        for j, f in enumerate(fills):
            ...
            border_points, approx_shape = get_border_point(
                f["point"], f["rect"], max_height, max_width
            )
