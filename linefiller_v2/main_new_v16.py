...
class ColorizationPipeline:
    ...
    def build_region_adjacency_graph(self, labels_map: np.ndarray):
        """Builds a graph on a dilated map to ensure topological correctness."""
        ...
        fill_mask = (labels_map > 0).astype(np.uint8)
        dilated_fills = cv2.dilate(fill_mask, kernel, iterations=2)
        # Reconstruct map with dilated fills but original labels
        logic_map = labels_map.copy()
        logic_map[dilated_fills == 1] = labels_map[dilated_fills == 1]
        logic_map[labels_map == 0] = 0
        return build_graph_numba(logic_map)
