...
def get_unfilled_points(image: np.ndarray, step: int = 5):
...
def merge_fill(fillmap: np.ndarray, max_iter: int = 3):
...
def process_image(image_path, output_dir, resize=None):
    ...
    if img.shape[2] == 4:
        ...
        _, line_art = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY_INV)

    # Prepare binary image
    _, binary = cv2.threshold(line_art, 220, 255, cv2.THRESH_BINARY_INV)
    ...
    for radius, method in zip(radii, methods):
    ...
    # Save fill map
    cv2.imwrite(
        str(output_dir / "fill_map.png"), show_fill_map(thinning(merged))
    )

    cv2.imwrite(str(output_dir / "colored.png"), show_fill_map(merged))
