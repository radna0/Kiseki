...
class ColorizationPipeline:
    ...
    def process(self, img_path: str) -> np.ndarray:
        ...
        # 1. Core segmentation using the superior watershed with guaranteed markers.
        resolved_map = self.watershed_segmentation(binary_art)

        # 2. Build the graph on the direct, unaltered output of the watershed.
        # The merge logic is robust enough to handle the raw output.
        region_graph = build_graph_numba(resolved_map)

        # 3. Run the robust iterative merge.
        merged_map = self.merge_regions_iteratively(resolved_map, region_graph)

        # 4. Remove any final pixel noise.
        despeckled_map = self.despeckle(merged_map)

        # 5. The final, absolute enforcement of the ground truth line art. This is the ultimate spill-guard.
