...
# --- The DEFINITIVE, high-quality, high-speed thinning function ---
@njit(nogil=True, fastmath=True)
def fast_modal_thinning(fillmap: np.ndarray, max_iter: int = 20):
    ...
    for _ in range(max_iter):
        ...
        for i in range(line_points_y.shape[0]):
            ...
            # Collect all valid, non-zero neighbors
            n_count = 0
            ...
            if n_count > 0:
                ...
                max_freq, mode = 0, -1
                active_neighbors = neighbors[:n_count]
                for k in range(n_count):
                    val = active_neighbors[k]
                    freq = np.sum(active_neighbors == val)
                    if freq > max_freq:
                        max_freq, mode = freq, val
                if mode != -1 and result[y, x] != mode:
                    updates.append((y, x, mode))
                    changed_in_pass = True

        # Apply all updates for this pass simultaneously to avoid directional bias
        if not changed_in_pass:
            break
        for y, x, mode in updates:
            result[y, x] = mode
...
class ColorizationPipeline:
    ...
    def watershed_segmentation(self, line_art_binary: np.ndarray):
        """Watershed segmentation with guaranteed markers for all regions."""
        print("[INFO] Performing Watershed with Guaranteed Markers...")
        num_components, all_components = cv2.connectedComponents(line_art_binary)
        print(f"[INFO] Found {num_components - 1} potential regions to segment.")
        markers = all_components.astype(np.int32) + 1
        markers[self.original_line_mask] = 0
        markers = cv2.watershed(self.original_bgr, markers)
        markers[markers == -1] = 0
        return markers

    def build_region_adjacency_graph(self, paint_map: np.ndarray):
        """Builds a graph on a dilated 'logic map' to ensure topological correctness."""
        print("[INFO] Building topologically-generous Region Adjacency Graph...")
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        # Create the temporary 'logic_map' by dilating fills to close 1-pixel gaps
        # This operation MUST be on a uint8 mask
        fill_mask = (paint_map > 0).astype(np.uint8)
        dilated_fills = cv2.dilate(fill_mask, kernel, iterations=1)

        # Build the graph on the dilated version to catch all adjacencies
        temp_logic_map = paint_map.copy()
        temp_logic_map[dilated_fills == 0] = 0  # Ensure lines are still barriers

        return build_graph_numba(temp_logic_map)
    ...
    def merge_regions_iteratively(self, labels_map: np.ndarray, graph: dict):
        ...
        num_labels, _, stats, _ = cv2.connectedComponentsWithStats(
            (labels_map > 0).astype(np.uint8), connectivity=8, ltype=cv2.CV_32S
        )
        areas = {
