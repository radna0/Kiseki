...
def compute_mask(img, color):
    # Assumes img.shape = (H, W, C) and len(color) == C.
    H, W, C = img.shape
...
def process_line(
    ...
    with Profiler("Extracting Segments Time", limit=20):
        extract_seg_from_color(seg_color_path, line_path, seg_path)

    with Profiler("Colorizing Segments Time", limit=20):
