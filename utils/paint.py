...
def labelpng_2_np(input_img):
    ...
    if len(input_img.shape) == 3 and input_img.shape[2] == 3:
        ...
        output_np = (
            (input_img[:, :, 0].astype(np.int32) << 16)
            + (input_img[:, :, 1].astype(np.int32) << 8)
            + input_img[:, :, 2].astype(np.int32)
        )
...
def generate_random_colors(num_colors, shuffle=True):
    ...
    colors = np.stack([r.flatten(), g.flatten(), b.flatten()], axis=-1) * step + int(
        step / 2
    )
    if shuffle:
...
def recolorize_seg(label_tensor, colorize_line=False):
    ...
    if len(labels) - 2 <= 0:
        ...
        colors = np.vstack(
            ([0, 0, 0], [255, 255, 255], generate_random_colors(len(labels) - 2))
        )
...
def recolorize_img(img):
    ...
    non_white_colors = [
        color
        for color in generate_random_colors(len(unique_colors) - 1)
...
def eval_json(input_json_path, gt_json_path, threshold=10):
    ...
    assert len(input_color_dict.keys()) == len(gt_color_dict.keys()), (
        "Input and output list length should match! " + input_json_path
    )
    ...
    for index in input_color_dict.keys():
        ...
        if input_color_dict[index] == gt_color_dict[index]:
            ...
            if index_counts[int(index)] > threshold:
                acc_thres_sum += 1
        if input_color_dict[index] == [255, 255, 255, 0] or gt_color_dict[index] == [
            255,
            255,
            255,
            0,
        ...
        if input_color_dict[index] == [255, 255, 255, 0] and gt_color_dict[index] == [
            255,
            255,
            255,
            0,
...
def eval_json_folder(
    input_folder_path, gt_folder_path, result_folder_name, threshold=10
):
...
def calculate_avg(data_list, split_interval=None, interval_dict={}):
    ...
    if split_interval == None:
        ...
        avg_dict["avg"] = sum(
            ...
            for group in (
                data_list[i : i + split_interval]
                for i in range(0, len(data_list), split_interval)
            )
        ) / (len(data_list) // 20)
    else:
        start_mark = 0
        ...
        for key in interval_dict:
            ...
            avg_dict[key] = sum(
                sum(group[1:split_interval]) / (split_interval - 1)
                for group in (
                    sub_data_list[i : i + split_interval]
                    for i in range(0, len(sub_data_list), split_interval)
                )
...
def evaluate(result_tuple, mode="Default", split_interval=None, save_path=None):
    ...
    output_avg_dict = {
        key: value / len(all_results_dict) for key, value in output_avg_dict.items()
    }
    if len(interval_dict) != 0:
        output_camera_dict = {
            outer_key: {
                inner_key: inner_value / len(all_results_dict)
...
def merge_color_line(line_path, colorized_img_path, save_path):
    ...
    if colorized_img.shape[2] == 3:
        colorized_img = np.dstack(
            [colorized_img, np.ones(colorized_img.shape[:2], dtype=np.uint8) * 255]
        )
    ...
    inpainted_image = 255 * inpaint.inpaint_biharmonic(
        colorized_img[..., :3] / 255, mask, channel_axis=-1
    )
    inpainted_image_alpha = np.dstack([inpainted_image, colorized_img[..., 3]])

    # Extract the black line from the line image
    blended_rgb = line_rgb * line_alpha[..., np.newaxis] + inpainted_image * (
        1 - line_alpha[..., np.newaxis]
    )
    final_alpha = np.clip(
        line_alpha + inpainted_image_alpha[..., 3] * (1 - line_alpha), 0, 1
    )
...
if __name__ == "__main__":
    ...
    colorized_img_path = (
        "dataset/PaintBucket_demo/tangyuan_anime/tangyuan_anime/0000.png"
    )
